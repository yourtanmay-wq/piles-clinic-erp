-- ============================================================================
-- V328 — RMP SAFE VIEW ALL FOUNDATION (ADDITIVE / NOT LIVE IN APP YET)
-- Keeps current legacy values exactly, including Paid above Bill.
-- No patient/RMP/payment/commission/expense row is changed.
-- ============================================================================

create or replace function fin.rmp_legacy_view_all(p_rmp_id text)
returns table(
...
revoke all on function fin.rmp_legacy_view_all(text) from public, anon;
grant execute on function fin.rmp_legacy_view_all(text) to authenticated;
notify pgrst, 'reload schema';

select
  has_function_privilege('authenticated','fin.rmp_legacy_view_all(text)','EXECUTE') as authenticated_can_call,
  has_function_privilege('anon','fin.rmp_legacy_view_all(text)','EXECUTE') as anon_can_call;
